...
public class PedidoControllerTest {
    ...
    void testListarTodos() throws Exception {
        ...
        Mockito.when(pedidoService.listarPedidos()).thenReturn(Arrays.asList(new Pedido(1L, 1L, 1L, 5, "estado", java.time.LocalDate.now(), "direccionEntrega")));
        Mockito.when(pedidoService.listarPedidos()).thenReturn(Arrays.asList(new Pedido(1L, 1L, 1L, 5, "estado", java.time.LocalDate.now(), "direccionEntrega")));
    ...
    void testActualizar() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/pedidos/1").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new PedidoDTO(1L, 1L, 1L, 5, "estado", java.time.LocalDate.now(), "direccionEntrega"))))
